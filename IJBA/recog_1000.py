...
def test_recog(model_type):
    ...
    for split in range(1):
        ...
        split_protocol_dir = protocol_dir + 'split1/' 
        #split_protocol_dirrr = os.path.join(protocol_dir, 'test')
        #split_img_dir = os.path.join(align_img_dir, 'split{}'.format(split))
        probe_file = split_protocol_dir + 'search_probe_1.csv'

        probe_data = load_meta_test_data(probe_file, 'probe')
        gallery_file = split_protocol_dir + 'search_gallery_'+ str(split) + '.csv'
        gallery_data = load_meta_data(gallery_file, 'gallery')

        # load extract feat
        feats = dict()
        feat_dim = 0
        split_img_dir = align_img_dir + 'split1/'
        for img_type in ['gallery', 'probe']:
            list_file = split_img_dir + str(img_type) + '_list.txt'
            bin_file = split_img_dir + str(model_type) + '_' + str(img_type) + '_feat.bin'
